<template>
  <section class="demo">
    ...
    <p class="note">
      ...
      a branch (a continent, a country) opens a nested level instead of committing a value; only a
      leaf (a city) can become the model. Hover-intent, ArrowRight/ArrowLeft, typeahead, and "select
      closes the whole chain" all come straight from <code>Menu</code>'s own submenu machinery — try
      it with a keyboard, exactly like the Menu demo above.
    </p>
    ...
    <h3>Standalone vs. wired to Field</h3>
    <p class="note">
    ...
    <h3>Empty tree</h3>
    <p class="note">An empty <code>items</code> array renders the localized empty row.</p>
    <CascadeSelect :items="[]" placeholder="Nothing to pick" />
  </section>
</template>
