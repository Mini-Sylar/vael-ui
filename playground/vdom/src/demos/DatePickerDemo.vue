<template>
  <section class="demo">
    <h2>DatePicker</h2>
    <p class="note">
      Select's own trigger/positioner/panel chrome wrapping a real <code>Calendar</code> instead of
      a listbox. Opens on click/focus, closes and shows the formatted date the moment a day is
      picked. Try it with a keyboard: Tab to the field, then ArrowDown/Enter to open — focus lands
      right on the grid, ready for the same Left/Right/Up/Down/PageUp/PageDown navigation as the
      standalone Calendar above.
    </p>

    <h3>Basic — <code>v-model</code></h3>
    <div class="row">
      <DatePicker v-model="basicValue" placeholder="Pick a date" />
      <output class="panel-text">
        {{ basicValue ? basicValue.toDateString() : 'Nothing selected yet' }}
      </output>
    </div>

    <h3>Min/max bounds + disabled dates</h3>
    <div class="row">
      <DatePicker
        v-model="boundedValue"
        :min-date="minDate"
        :max-date="maxDate"
        :disabled-dates="isWeekend"
        placeholder="Weekdays this month only"
      />
    </div>

    <h3>Field-wrapped — label/description/required/error wiring</h3>
    <p class="note">Same "standalone-or-Field" contract as every other control here.</p>
    <Field
      label="Departure date"
      description="Required for one-way bookings"
      :required="true"
      :error="fieldError"
    >
      <DatePicker
        v-model="fieldValue"
        placeholder="Choose a departure date"
        @change="onFieldChange"
      />
    </Field>

    <h3>Disabled</h3>
    <div class="row">
      <DatePicker disabled placeholder="Unavailable" />
    </div>

    <h3>Range — <code>selectionMode="range"</code></h3>
    <p class="note">
      The trigger shows both ends once committed; the panel stays open after the first click since
      there's a second date still to pick.
    </p>
    <div class="row">
      <DatePicker v-model="rangeValue" selection-mode="range" placeholder="Pick a date range" />
    </div>

    <h3>Month-only / year-only — <code>view</code></h3>
    <div class="row">
      <DatePicker v-model="monthValue" view="month" placeholder="Pick a month" />
      <DatePicker v-model="yearValue" view="year" placeholder="Pick a year" />
    </div>
  </section>
</template>

<script setup lang="ts">
import { computed, shallowRef } from 'vue'
import { DatePicker, Field } from 'vael-ui'
import type { CalendarRange } from 'vael-ui'

const basicValue = shallowRef<Date | null>(null)
const boundedValue = shallowRef<Date | null>(null)
const fieldValue = shallowRef<Date | null>(null)
const rangeValue = shallowRef<CalendarRange | null>(null)
const monthValue = shallowRef<Date | null>(null)
const yearValue = shallowRef<Date | null>(null)

const today = new Date()
const minDate = new Date(today.getFullYear(), today.getMonth(), 1)
const maxDate = new Date(today.getFullYear(), today.getMonth() + 1, 0)
function isWeekend(date: Date): boolean {
  const day = date.getDay()
  return day === 0 || day === 6
}

const fieldError = computed(() => (fieldValue.value ? '' : 'Pick a departure date to continue'))
function onFieldChange() {
  // Demonstrates @change event fires
}
</script>
