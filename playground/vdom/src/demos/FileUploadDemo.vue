<template>
  <section class="demo">
    ...
    <p class="note">
      ...
      <code>useFileDrop</code> (the headless composable underneath) is exported and attachable to
      any element — see the Composer demo above, which attaches this exact same composable to a
      <code>Textarea</code>.
    </p>

    <h3>Simulated upload — a <code>Progress</code> bar per file</h3>
    <p class="note">
      ...
      <template #item="{ file, remove }">
        <div class="upload-item">
          <div class="upload-item-row">
            ...
            <button type="button" class="upload-item-remove" aria-label="Remove" @click="remove">
              ×
            </button>
          </div>
    ...
    <h3>Not multiple — replaces on each pick</h3>
    <FileUpload v-model:files="singleFile" :multiple="false" />
    ...
    <p class="note">
      ...
      on top of (not instead of) the library's own opacity fade on the <code>&lt;li&gt;</code>. This
      is the <code>#item</code> slot pattern: the library still owns the row's mount/unmount and its
      own transition, the consumer only reaches the content inside via a function ref, never an
      internal class.
    </p>
...
import { FileUpload, Progress, toast } from 'vael-ui'
import type { FileRejectReason } from 'vael-ui'

const uploadFiles = shallowRef<File[]>([])
const singleFile = shallowRef<File[]>([])
