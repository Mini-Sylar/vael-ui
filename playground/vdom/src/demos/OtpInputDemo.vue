<template>
  <section class="demo">
    ...
    <p class="note">
      The <code>#cell</code> slot swaps a cell's markup; geometry stays the library's.
    </p>
    <OtpInput v-model="custom" :length="4">
      <template #cell="{ char, active, filled }">
        <span :class="['custom-otp-cell', { active, filled }]">{{ char ?? '·' }}</span>
      </template>
    </OtpInput>
...
.custom-otp-cell.active {
  border-style: solid;
  border-color: var(--ui-primary);
}
.custom-otp-cell.filled {
