<template>
  <section class="demo">
    <h2>SplitButton</h2>
    <p class="note">
      A primary action fused with a chevron that opens alternate actions, "Save" plus "Save As…" or
      ...
      <code>#trigger</code> slot. Two separately focusable tab stops, not one fake compound widget,
      Tab from the main action lands on the chevron, and Menu's own keyboard handling (arrows,
      typeahead, Home/End) works unmodified once wired to it.
    </p>

    <h3>Basic usage: main click vs. chevron dropdown</h3>
    <div class="row">
      <SplitButton :items="saveItems" @click="onSave" @select="onSaveOption">Save</SplitButton>
      <output class="panel-text">{{ lastAction }}</output>
    </div>

    <h3>Auto-loading main action</h3>
    <p class="note">
      The main action forwards straight onto its own inner <code>Button</code>, so returning a
      promise from <code>@click</code> drives Button's loading state with zero extra wiring, exactly
      like a plain <code>&lt;Button @click&gt;</code> would.
    </p>
    <div class="row">
    ...
    <div class="row">
      ...
      <SplitButton disabled :items="saveItems">disabled</SplitButton>
    </div>
    <div class="row">
