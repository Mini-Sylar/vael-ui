<template>
  <section class="demo">
    ...
    <p class="note">
      ...
      own center (<code>getBoundingClientRect()</code>) and maps it onto a fixed 270deg sweep (<code
        >-135deg</code
      >
      to <code>135deg</code>, a 90deg gap at the bottom), the same "never transition a live drag"
      rule as Slider applies to the rotation angle and fill arc. Grab anywhere on the dial: the
      value jumps to that angle immediately, then drag, the indicator tracks 1:1, no lag.
    </p>

    <h3>Custom range and step, with <code>valueText</code></h3>
...
import { Field, Knob } from 'vael-ui'
